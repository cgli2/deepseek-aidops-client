...
fn inner_stream_chat(
    ...
    Box::pin(stream! {
        ...
        loop {
            ...
            collect_stream_tool_fragments(&v, &mut tools);
        }
        ...
        if !tool_calls.is_empty() {
            ...
            if matches!(finish_reason.as_deref(), Some("tool_calls" | "function_call")) {
                yield Err(Error::Llm(format!(
                    "{provider_label} 声明 finish_reason=tool_calls，但响应中没有可解析的工具名称和参数；已停止同请求重试，请检查网关的 tool_calls 流式兼容格式"
                )));
                return;
            }
            yield Ok(Chunk {
...
type StreamTools = BTreeMap<u64, (String, String, String)>;

/// 收集标准 OpenAI 增量、终止帧完整 message、旧式 function_call，以及常见
/// 兼容网关的对象参数。终止帧快照只补缺失字段，避免把已收到的增量重复拼接。
fn collect_stream_tool_fragments(frame: &Value, tools: &mut StreamTools) {
    let Some(choice) = frame.pointer("/choices/0") else {
        return;
    };
    if let Some(delta) = choice.get("delta") {
        collect_tool_payload(delta, tools, false);
    }
    if let Some(message) = choice.get("message") {
        collect_tool_payload(message, tools, true);
    }
}

fn collect_tool_payload(payload: &Value, tools: &mut StreamTools, snapshot: bool) {
    if let Some(calls) = payload.get("tool_calls").and_then(Value::as_array) {
        for (position, call) in calls.iter().enumerate() {
            ...
            collect_one_tool(call, index, tools, snapshot);
        }
        return;
    }
    if let Some(function) = payload.get("function_call") {
        collect_one_tool(function, 0, tools, snapshot);
    }
}

fn collect_one_tool(call: &Value, index: u64, tools: &mut StreamTools, snapshot: bool) {
    ...
    let arguments = function
        ...
        .and_then(json_fragment)
        .unwrap_or_default();
    let entry = tools
        .entry(index)
        .or_insert_with(|| (id.clone(), String::new(), String::new()));
    if (snapshot || entry.0.starts_with("call_")) && !id.starts_with("call_") {
        entry.0 = id;
    }
    if snapshot {
        if !name.is_empty() {
...
fn json_fragment(value: &Value) -> Option<String> {
    match value {
...
mod tests {
    ...
    fn messages_json_encodes_user_images_as_openai_content_parts() {
        let message =
            Message::user_with_images("请识别图片", vec!["data:image/png;base64,aGVsbG8=".into()]);
    ...
    #[test]
    fn stream_tool_parser_accepts_terminal_message_and_object_arguments() {
                ...
                "choices": [{
                    ...
                    "message": {
                        "tool_calls": [{
                            ...
                            "function": {
                                "name": "edit",
                                "arguments": {"path": "src/menu.rs", "new": "智能体装配"}
                            }
                        }]
                    }
                }]
        ...
        let calls = finish_stream_tool_calls(tools).unwrap();
        assert_eq!(calls[0].id, "call-menu");
        assert_eq!(calls[0].name, "edit");
        assert_eq!(calls[0].args["new"], "智能体装配");
    }

    #[test]
    fn stream_tool_parser_accepts_legacy_function_call_and_string_index() {
        let mut tools = StreamTools::new();
        collect_stream_tool_fragments(
            &json!({
                "choices": [{"delta": {
        ...
        assert_eq!(calls[0].args["pattern"], "多端拼装");

        let mut fragmented = StreamTools::new();
        for frame in [
            json!({"choices":[{"delta":{"tool_calls":[{"index":"0","id":"c1","function":{"name":"fs","arguments":"{\"op\":"}}]}}]}),
            json!({"choices":[{"delta":{"tool_calls":[{"index":"0","function":{"arguments":"\"read\"}"}}]}}]}),
        ] {
            collect_stream_tool_fragments(&frame, &mut fragmented);
        }
        let calls = finish_stream_tool_calls(fragmented).unwrap();
        assert_eq!(calls[0].args["op"], "read");
    }

    #[test]
    fn malformed_stream_tool_arguments_fail_closed() {
        let mut tools = StreamTools::new();
        collect_stream_tool_fragments(
